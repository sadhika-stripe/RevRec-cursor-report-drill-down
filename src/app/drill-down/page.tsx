...
export default function DrillDownPage() {
  ...
  const drillDownData = {
    ...
    entries: [
      ...
      ...entry,
      eventType: entry.eventType === "Placeholder event type"
        ? eventTypes[idx % eventTypes.length]
        : entry.eventType
    })),
    total: {
      debit: "$22,840.00",
  ...
  // Trigger chart animation when showChart becomes true
  useEffect(() => {
    if (showChart) {
  ...
  function parseCurrency(str: string): number {
    if (!str) return 0;
    return parseFloat(str.replace(/[^\d.-]/g, '')) * (str.includes('(') ? -1 : 1);
  }

  // Get the target total from the top (amount)
  const targetTotal = parseCurrency(amount);

  // Calculate running totals for debits and credits, excluding the last row
  const debits = drillDownData.entries.map(e => parseCurrency(e.debit));
  const credits = drillDownData.entries.map(e => parseCurrency(e.credit));
  const sumDebits = debits.slice(0, -1).reduce((a, b) => a + b, 0);
  const sumCredits = credits.slice(0, -1).reduce((a, b) => a + b, 0);

  // Adjust the last debit or credit so that totalDebits + totalCredits = targetTotal
  let lastDebit = 0;
  let lastCredit = 0;
  const currentDebits = sumDebits;
  const currentCredits = sumCredits;
  // Try to adjust lastDebit first
  let calcLastDebit = targetTotal - currentDebits - currentCredits;
  if (calcLastDebit >= 0) {
    lastDebit = calcLastDebit;
    lastCredit = 0;
  } else {
    lastDebit = 0;
    lastCredit = targetTotal - currentDebits - currentCredits;
  }
  // Calculate totals for display
  const totalDebits = currentDebits + lastDebit;
  const totalCredits = currentCredits + lastCredit;
  ...
  // Helper to parse date strings like 'Dec 1, 10:00 AM' to Date objects (assuming current year)
  function parseEntryDate(dateStr: string): Date {
    // Example: 'Dec 31, 10:00 AM' => 'Dec 31, 2024 10:00 AM'
    const year = new Date().getFullYear();
    return new Date(`${dateStr}, ${year}`);
  }

  // Sort entries in descending order by date (latest first)
  const sortedEntries = drillDownData.entries.slice().sort((a, b) => parseEntryDate(b.date).getTime() - parseEntryDate(a.date).getTime());

  // Render entries with adjusted last row
  const entriesToRender = sortedEntries.map((entry, idx, arr) => {
    if (idx === arr.length - 1) {
      return {
        ...entry,
        debit: lastDebit !== 0 ? formatCurrency(lastDebit) : '',
        credit: lastCredit !== 0 ? formatCurrency(lastCredit) : '',
      };
    }
    return entry;
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col h-screen">
        {/* Page Header - Fixed */}
        <div className="bg-white border-b border-gray-200 px-6 py-2 flex-shrink-0 z-20">
          ...
          {showChart && (
            <div className="mt-4 p-6 bg-white rounded border">
              ...
              <div className={`mt-4 flex items-center gap-2 text-sm text-gray-600 transition-all duration-500 ${
                ...
                <span>
                  {showForecast 
                    ? "Peak activity on Dec 3rd with $2.1K. Forecast shows steady decline through month-end, suggesting seasonal payment patterns."
                    : "Peak activity on Dec 3rd with $2.1K in receivables. Daily average of $1.8K shows consistent collection performance."
                  }
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto bg-white">
            <table className="w-full table-fixed">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {entriesToRender.map((entry: any, index: number) => (
        ...
        <div className="bg-gray-50 border-t border-gray-200 flex-shrink-0 sticky bottom-0">
          <table className="w-full table-fixed">
            ...
            <tbody>
              <tr>
                <td className="px-6 py-4 text-sm font-medium text-gray-500 inline-flex items-center gap-1">
                  Total
                  <div className="relative group inline-flex items-center align-middle">
                    <span className="ml-0.5 cursor-pointer align-middle">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400 align-middle" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 16v-4m0-4h.01" /></svg>
                    </span>
                    {/* For a robust solution, use a portal. This is a quick fix for now. */}
                    <div className="fixed left-10 bottom-10 w-80 max-w-xs bg-white border border-gray-200 rounded shadow-lg text-xs text-gray-700 px-3 py-2 whitespace-pre-line opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                      <div className="mb-2 text-sm text-gray-800">
                        The sum of debits and credits gives the net change in {title.toLowerCase()}.
                      </div>
                      <div className="bg-gray-50 rounded px-3 py-1 font-mono text-sm text-gray-800 flex items-center justify-center space-x-2">
                        <span>{formatCurrency(totalDebits)}</span>
                        <span>-</span>
                        <span>{formatCurrency(Math.abs(totalCredits))}</span>
                        <span>=</span>
                        <span className="font-bold">{amount.replace(' USD', '')}</span>
                      </div>
                    </div>
                  </div>
                </td>
